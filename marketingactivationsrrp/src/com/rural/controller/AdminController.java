...
public class AdminController {
	...
	public @ResponseBody ConversionMaster getConvRowData(@RequestParam("id") int strId,HttpServletRequest request, HttpServletResponse response,ModelMap model){
	...
	public @ResponseBody SaturationMaster  getSatRowData(@RequestParam("id") int strId,HttpServletRequest request, HttpServletResponse response,ModelMap model){
	...
	public @ResponseBody  ConversionMaster updateConv(@RequestParam("id") int strConvId,@RequestParam("conv") String strConversion,
...
result = adminService.updateConv(strConvId, strConversion,"",ipAddress);
	...
	public @ResponseBody  SaturationMaster updateSat(@RequestParam("id") int strId,@RequestParam("newSat") String strSat,
			HttpServletRequest request, HttpServletResponse response){
		...
		try{
			...
			if(result == 1) 
		{
			//DB Updated Succesfully so send this updated Saturation Data to Update Table Cell
...
/*			logger.error("Some Problem Occured while downloading Saturaions "+erp.getMessage());*/
		} catch (Exception e) {
			/*logger.error("Some Problem Occured while downloading Saturaions "+e.getMessage());*/
		}
	}
	...
	public void exportSaturation(HttpServletResponse response, List<SaturationMaster> itemList,String strFile) {
		...
		try {
			...
			while (iter.hasNext()) {
				...
				rows.add(item.getBrand()+","+item.getState()+","+item.getCity()+","
				+item.getSatpercent()+","+item.getLastUpdated()+","+item.getUpdatedBy());
			...
			/*logger.error("Some Problem Occured while downloading Saturaions "+ie.getMessage());*/
		} catch (Exception e) {
			/*logger.error("Some Problem Occured while downloading Saturaions "+e.getMessage());*/
		} finally {
			try {
	...
	public @ResponseBody int saveNewAgency(@ModelAttribute("agency")AgencyMaster agencyMaster,HttpServletRequest request,
			Map<String, Object> model) {
		@SuppressWarnings("unused")
		//String agencyname=request.getParameter("agencyname");
		//int state=Integer.parseInt(request.getParameter("state"));
		//int city=Integer.parseInt(request.getParameter("city"));
		AgencyMaster agencyMaster1=new AgencyMaster();
...
*/
	...
	public @ResponseBody String lockVendor(@RequestParam("venId") int strId,HttpServletRequest request,HttpServletResponse responce) {
	...
	public @ResponseBody String unlockVendor(@RequestParam("venId") int strId,
	...
	public @ResponseBody String lockAgency(@RequestParam("agenId") int strId,
			HttpServletRequest request,HttpServletResponse responce) {
	...
	public @ResponseBody String unlockAgency(@RequestParam("agenId") int strId,
			HttpServletRequest request,HttpServletResponse responce) {
		String strStatus=null;
		try {
			int result =0;
	...
	public @ResponseBody String lockUser(@RequestParam("userId") int strId,
			HttpServletRequest request,HttpServletResponse responce) {
		String strStatus=null;
		try {
			int result =0;
	...
	public @ResponseBody String unlockUser(@RequestParam("userId") int strId,
			HttpServletRequest request,HttpServletResponse responce) {
		String strStatus=null;
		try {
			int result =0;
	...
	public @ResponseBody Map<Integer,String> getCityState(@RequestParam("state") Integer intStateId, HttpServletRequest request,
			Model model) {
		...
		CityMaster cityMaster=null;
		 while(cityItr.hasNext()){
			...
			/*logger.error("Problem Occured while loading cities. Please try Again "+erp.getMessage());*/
		} catch (Exception e) {
			/*logger.error("Problem Occured while loading cities. Please try Again "+e.getMessage());*/
		}
		return cityMap;
	...
	public @ResponseBody Map<Integer,String> getVendorofAgency(@RequestParam("agency") Integer agencyId,
		...
		VendorMaster vendorMaster=null;
		 while(vendorItr.hasNext()){
			...
			/*logger.error("Problem Occured while loading vendors. Please try Again "+erp.getMessage());*/
		} catch (Exception e) {
			/*logger.error("Problem Occured while loading vendors. Please try Again "+e.getMessage());*/
		}
		return vendorMap;
	...
	public @ResponseBody AgencyMaster getAgencyRowData(@RequestParam("id") int strId, HttpServletRequest request,
		...
		try{
			...
			/*logger.error("Problem Occured while loading agency. Please try Again "+erp.getMessage());*/
		} catch (Exception e) {
			/*logger.error("Problem Occured while loading agency. Please try Again "+e.getMessage());*/
		}
		return agencyMaster;
	}
	...
	public void downloadAgencyMaster(HttpServletRequest request,HttpServletResponse response) {
		...
		/*logger.error("Problem Occured while downloading agencies. Please try Again "+erp.getMessage());*/
	} catch (Exception e) {
		/*logger.error("Problem Occured while downloading agencies. Please try Again "+e.getMessage());*/
	}
	}

	@SuppressWarnings("unchecked")
	public void exportAgencyMaster(HttpServletResponse response, List<AgencyMaster> itemList,String strFile,HttpServletRequest req) {
		ServletOutputStream outStream = null;
		...
		try {
			...
			while (iter.hasNext()) {
				...
				rows.add(item.getAgencyname()+","+stateMap.get(item.getStateId())+","+cityMap.get(item.getCityId())+","+item.getIsActive());
				rows.add("\n");
			}
			outStream = response.getOutputStream();
			response.setContentType("text/csv");
	...
	public void exportVendorMaster(HttpServletResponse response, List<VendorMaster> itemList, String strFile,HttpServletRequest req) {
		ServletOutputStream outStream = null;
		Map<Integer, String> stateMap=new HashMap<Integer, String>();
			Map<Integer, String> cityMap=new HashMap<Integer, String>();
			...
			while (iter.hasNext()) {
				...
				rows.add(item.getVendorName() + "," + agencyMap.get(item.getAgencyId()) + "," + stateMap.get(item.getState()) + "," + cityMap.get(item.getCity())
						+ "," + item.getIsActive());
	...
	public ModelAndView createUserForm( ModelMap map,HttpServletRequest request) {
	...
	public ModelAndView showUsersMaster( ModelMap model,HttpServletRequest request) {
	...
	public ModelAndView UpdateUserDetails(HttpServletRequest request,ModelMap map) {
		...
		if (role==5) {
			Integer strBrand = request.getParameter("brand") == null ? 0 : Integer.parseInt(request.getParameter("brand"));
			user.setBrandId(strBrand);
		} else if (role==4) {
			String strMobileAcess = request.getParameter("mobileaccess") == null ? "No": request.getParameter("mobileaccess");
	...
	public void exportUserMaster(HttpServletResponse response, List<UserMaster> itemList, String strFile,HttpServletRequest req) {
		ServletOutputStream outStream = null;
			Map<Integer, String> stateMap=new HashMap<Integer, String>();
			Map<Integer, String> cityMap=new HashMap<Integer, String>();
			Map<Integer, String> agencyMap=new HashMap<Integer, String>();
...
*/				
			...
			while (iter.hasNext()) {
				...
				rows.add(item.getUsername() + "," + item.getFirstName() + "," + item.getLastName() + ","
						...
						+ agencyMap.get(item.getAgencyId()) + "," + vendorMap.get(item.getVendorId()) + "," + item.getMobileAccess()
						+ ","/*
